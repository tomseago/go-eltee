...
// is appropriate. While this theoretically could be a different rate than the frames
// are being generated by the rest of the server, in reality it's probably not. Thus
// this output go routine is the main pump loop of the server.
...
func (h *DmxHarness) Start() {
	...
	// Right now we are just delaying a fixed time. In the future we will want
	// to either vary this delay or the main server update code needs to run
	// in a render routine that synchronizes with the "copy from backing store"
